...
const createImage         = async (req, res, next) => { try { res.status(201).json(await medicalImageService.createImage(req.body)); } catch (err) { next(err); } };
...
const deleteImage         = async (req, res, next) => { try { await medicalImageService.deleteImage(req.params.id); res.status(204).send(); } catch (err) { next(err); } };
...
const addRadiologyNote     = async (req, res, next) => { try { res.status(201).json(await medicalImageService.addRadiologyNote(req.params.id, req.user.id, req.body)); } catch (err) { next(err); } };
const updateRadiologyNote  = async (req, res, next) => { try { res.status(200).json(await medicalImageService.updateRadiologyNote(req.params.nid, req.body)); } catch (err) { next(err); } };
